...
修改包括统一路由、24 个英文模块入口、修复引用、合并通用授权规则、简化普通文档创作、限制非请求的妙搭发布，以及 uv 依赖管理和维护检查。清理幻灯片的强制设计偏好，修复日程嵌套重叠检查、会议内容总结路径和 Wiki 同名空间分页问题。命令说明中的版本号、参数和平台限制继承自上游基线；真实使用遇到差异时以当前 CLI 的精确 help/schema 为准。

详细参考、XML 说明和代码注释使用用户指定的 DeepSeek 模型逐文件英文化，命令、参数和协议字面量通过自动比对核验。保留中文用户文档、界面文案、示例数据及旧章节锚点。按用户要求，人工仅抽查邮件发送、表格条件格式、幻灯片创建三份参考，并修正抽查发现的重复确认和无条件上色被误强制为条件格式的问题；不声称逐份人工审校或全量线上验收。
