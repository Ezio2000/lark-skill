...
> 本节给出"写公式 / 排序 / 筛选前先探清列格式多样性"的正确流程，是主 index.md「飞书表格编辑准则」准则 3（读全再写）在 read_data 工具层的落地。
...
输出形状对齐 pandas split：`columns` 是列名数组、`data` 是二维数据、`dtypes` 是 `{列名: pandas_dtype_str}` 映射；`truncated/complete/truncation_warning` 说明覆盖度。未截断时可直接喂给 `pd.DataFrame(...).astype(...)`。本模块 提供 [`scripts/lark_sheets_df.py`](../scripts/lark_sheets_df.py)：
