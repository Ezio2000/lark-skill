...
按 [`lark-drive`](../../drive/index.md) 的路径和写操作规则执行 `drive +upload`，取得 `file_token`。上传参数和文件限制见 [`lark-drive-upload`](../../drive/references/lark-drive-upload.md)。
...
3. 用户只提供目标姓名时，用 [`lark-contact`](../../contact/index.md) 解析为 `ou_` open_id。
...
用户要查看妙记已授权给哪些成员，或查询某个成员当前的查看 / 编辑权限时，使用 Drive 协作者列表；这不是读取妙记内容，也不是为当前身份申请权限。先读取 [`lark-drive`](../../drive/index.md) 和 [`drive +member-list`](../../drive/references/lark-drive-member-list.md)。
...
先读取 [`lark-drive`](../../drive/index.md) 和 [`drive +member-add`](../../drive/references/lark-drive-member-add.md)。目标成员只有展示名时，按 [`lark-contact`](../../contact/index.md) 将其唯一解析为对应 ID；存在多个候选时请用户选择，不得猜测。
