...
本模块 对应 shortcut：`lark-cli vc +recording`。
...
| `missing required scope(s)` | 权限不足 | `--as user`：按提示运行 `auth login --scope`；`--as bot`：使用错误中的 `console_url` 去开发者后台开通，**禁止**对 bot 执行 `auth login`（见 [lark-shared](../../shared/index.md) 的权限管理） |
