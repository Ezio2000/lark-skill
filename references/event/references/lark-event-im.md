...
> **Prerequisite:** Read [`../index.md`](../index.md) first for the `event consume` essentials (commands, subprocess contract, jq usage).
>
...
| `card.action.trigger` | Interactive card callback — button click, form submit, dropdown, etc. → see [`lark-im-card-action-reply.md`](../../im/references/lark-im-card-action-reply.md) |
...
**`.content` shape depends on `message_type`** (this key uses a flat Custom schema; the CLI source implements it in `events/im/message_receive.go`, which is not bundled with this skill):
