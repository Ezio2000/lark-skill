...
| `copy_starter_component` | `Bash cp <本模块 所在目录>/starter-components/<file> .`（cwd 通常是应用项目目录而非 skill 目录，需用 skill 目录实际路径；或读取后改编）。 |
| 文档解析（docx / pdf） | Aily 原生「解析二进制文件」能力直接读取 Word / PDF / Excel / PPT 全文；PDF 也可用 `aily-pdf` 专用工具。 |
| `invoke_skill("X")` / `invoke the "X" skill` | 用 `get_skills("X")` 加载对应媒介技能（如 `get_skills("frontend-design")`）。这些技能同时以本地文件形式随本模块 附带在 `references/<X>.md`，`get_skills` 取不到时直接读该文件。 |
...
- `agent` 的 `slide` 子类型用于生成**飞书幻灯片**，与本模块 产出的自包含 HTML deck（`starter-components/deck-stage.js`）是两条不同路径，不要混用——本模块 的 deck 始终是 HTML。
