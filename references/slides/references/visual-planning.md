...
For new decks or major rewrites, read this after planning and before XML. Translate `layout_type`, `visual_focus`, and `text_density` into actual geometry rather than labels only.

Plan on a `960 x 540` canvas. User templates/styles override generic layout preferences; preserve readable hierarchy, fitting text, and intentional geometry. Minimal or text-led slides do not require images or cards.
...
- Around `40` px is a useful default outer margin. Choose density and whitespace for the requested style and content; do not force a card grid. Use full-bleed only intentionally.
...
- Use shapes/lines for diagrams, not as counterfeit photographs or evidence. Generated illustrations are optional when appropriate and available.
- Add rules, bands, or accents only when they serve the chosen design; omit meaningless decoration.
...
- Visual focus may be typography, a real or generated image, a native diagram, or a metric, depending on the brief. A no-image cover is valid; do not force an image search or generation.
...
- If no image is available, follow the planned fallback or choose a text/diagram layout consistent with the brief. Do not fabricate evidence screenshots or paper figures.
